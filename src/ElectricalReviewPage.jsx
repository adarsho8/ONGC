...
//     &params={Laq_Status,electricalReviewComment,electricalReviewSignature}&parts=all
// ─────────────────────────────────────────────────────────────────────────────

const AUTH   = "Basic " + btoa("bawadmin:sarasu10");
const ACCENT = "#d97706";   // electrical = amber
const LIGHT  = "#fffbeb";
...
async function completeTask(taskId, laqStatus, comment, signature) {
  const params = JSON.stringify({
    Laq_Status:                laqStatus,
    electricalReviewComment:   comment,
    electricalReviewSignature: signature,
  });
...
// ─────────────────────────────────────────────────────────────────────────────
export default function ElectricalReviewPage() {
  const [searchParams] = useSearchParams();
  const navigate       = useNavigate();

  const taskId   = searchParams.get("taskId")   || "";
  const taskName = searchParams.get("taskName") || "Electrical Review";
  ...
  const [form, setForm] = useState({
    ...
    sitesurveyComment: "",
    commandResponse: {},
    electricalReviewComment:   "",
    electricalReviewSignature: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await completeTask(taskId, form.laqStatus, form.electricalReviewComment, sigData);
      setSubmitState("done");
  ...
  if (fetchState === "loading") return <LoadingScreen taskId={taskId} accent={ACCENT} />;
  ...
  return (
    <div style={s.page}>
          ...
          <div style={s.card}>
            ...
            <div style={{ background: "linear-gradient(135deg, #0d0d0d, #1a1a1a)", padding: "24px 32px", borderBottom: `3px solid ${ACCENT}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 10, padding: "8px 14px", fontSize: 22 }}>⚡</div>
                <div>
                  ...
                  <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, marginTop: 3 }}>
                    Task #{taskId} · Electrical Review · Submit to complete and advance workflow
                  </p>
                </div>
              </div>
            </div>

            <div style={{ background: LIGHT, borderBottom: `1px solid #fde68a`, padding: "10px 32px", fontSize: 12, color: "#92400e", fontWeight: 600 }}>
              ✅ All site survey data loaded — review below and add your electrical assessment
            </div>

            {submitState === "done"  && <Alert type="success" msg="✅ Electrical Review completed! Token advanced in BAW. Returning to worklist…" />}
            ...
            <form onSubmit={handleSubmit} style={{ padding: "28px 32px" }}>
              ...
              <SectionHeader icon="⚡" title="Electrical Review Assessment" subtitle="Your review — required fields" accent={ACCENT} />
              <div style={{ display: "grid", gap: 20 }}>
                <div>
                  <label style={s.label}>
                    Electrical Review Comment <span style={s.reqTag}>* required</span>
                  </label>
                  <textarea name="electricalReviewComment"
                    value={form.electricalReviewComment} onChange={handleChange}
                    rows={4} required
                    placeholder="Enter electrical review findings, load assessments, and recommendations…"
                    style={{ ...s.input, resize: "vertical" }} />
                </div>

                <div>
                  <label style={s.label}>
                    Electrical Review Signature <span style={s.reqTag}>* draw below</span>
                  </label>
                  <div style={{ border: `2px solid #fde68a`, borderRadius: 10, overflow: "hidden", background: "#f8fafc" }}>
                    <canvas
                      ref={sigCanvasRef}
                      width={836} height={120}
                      style={{ display: "block", width: "100%", cursor: "crosshair", touchAction: "none" }}
                    />
                    <div style={{ background: LIGHT, padding: "6px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: `1px solid #fde68a` }}>
                      <span style={{ fontSize: 11, color: "#64748b" }}>
                        {hasSig ? "✏️ Signature captured" : "✏️ Draw your signature above"}
                      </span>
                      <button type="button" onClick={clearSig} style={{ fontSize: 11, color: ACCENT, background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>Clear</button>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {activeTab === "Document Upload" && <DocUploadTab taskId={taskId} accent={ACCENT} />}
    </div>
  );
}

// ── Shared components (same pattern as Civil) ─────────────────────────────────
...
function CommandResponseBlock({ data }) {
  if (!data || Object.keys(data).length === 0)
    return <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "16px 20px", fontSize: 13, color: "#94a3b8", fontStyle: "italic", marginBottom: 24 }}>No command response data available</div>;
  return (
...
function Alert({ type, msg }) {
  ...
  return (
    <div style={{ margin: "16px 32px 0", background: ok ? "#f0fdf4" : "#fff1f2", borderLeft: `4px solid ${ok ? "#16a34a" : "#dc2626"}`, borderRadius: 8, padding: "14px 18px", border: "1px solid #e2e8f0", color: ok ? "#15803d" : "#7f1d1d", fontSize: 13, fontWeight: ok ? 600 : 400 }}>
      {ok ? msg : <><div style={{ fontWeight: 700, color: "#dc2626" }}>⚠ Submission failed</div><pre style={{ fontSize: 11, marginTop: 6, whiteSpace: "pre-wrap", wordBreak: "break-all" }}>{msg}</pre></>}
    </div>
  );
}

function LoadingScreen({ taskId, accent }) {
  return (
    <div style={s.page}>
