...
//     &params={Laq_Status,sstReviewComment,sstReviewSignature}&parts=all
// ─────────────────────────────────────────────────────────────────────────────

const AUTH   = "Basic " + btoa("bawadmin:sarasu10");
const ACCENT = "#0f766e";   // SST = teal
const LIGHT  = "#f0fdfa";
...
async function completeTask(taskId, laqStatus, comment, signature) {
  const params = JSON.stringify({
    SSTreviewComment:   comment,
    SSTreviewSignature: signature,
  });
...
// ─────────────────────────────────────────────────────────────────────────────
export default function SSTReviewPage() {
  const [searchParams] = useSearchParams();
  const navigate       = useNavigate();

  const taskId   = searchParams.get("taskId")   || "";
  const taskName = searchParams.get("taskName") || "SST Review";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await completeTask(taskId, null, form.sstReviewComment, sigData);
      setSubmitState("done");
        ...
        <div style={{ display: "flex", justifyContent: "center", padding: "24px 16px 48px" }}>
          <div style={s.card}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  ...
                  <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, marginTop: 3 }}>
                    Task #{taskId} · SST Review · Submit to complete and advance workflow
                  </p>
                </div>
              </div>
            </div>

            <div style={{ background: LIGHT, borderBottom: `1px solid #99f6e4`, padding: "10px 32px", fontSize: 12, color: "#0f766e", fontWeight: 600 }}>
              ✅ All site survey data loaded — review below and add your SST assessment
            </div>

            {submitState === "done"  && <Alert type="success" msg="✅ SST Review completed! Token advanced in BAW. Returning to worklist…" />}
            ...
            <form onSubmit={handleSubmit} style={{ padding: "28px 32px" }}>
              ...
              <SectionHeader icon="🛡️" title="SST Review Assessment" subtitle="Your review — required fields" accent={ACCENT} />
              <div style={{ display: "grid", gap: 20 }}>
                <div>
                  <label style={s.label}>
                    SST Review Comment <span style={s.reqTag}>* required</span>
                  </label>
                  <textarea name="sstReviewComment"
                    value={form.sstReviewComment} onChange={handleChange}
                    rows={4} required
                    placeholder="Enter SST review findings, safety observations, and recommendations…"
                    style={{ ...s.input, resize: "vertical" }} />
                </div>

                <div>
                  <label style={s.label}>
                    SST Review Signature <span style={s.reqTag}>* draw below</span>
                  </label>
                  <div style={{ border: `2px solid #99f6e4`, borderRadius: 10, overflow: "hidden", background: "#f8fafc" }}>
                    <canvas
                      ref={sigCanvasRef}
                      width={836} height={120}
                      style={{ display: "block", width: "100%", cursor: "crosshair", touchAction: "none" }}
                    />
                    <div style={{ background: LIGHT, padding: "6px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: `1px solid #99f6e4` }}>
